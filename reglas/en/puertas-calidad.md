...
| # | Gate | Phase | Criteria |
|---|------|-------|----------|
| 1 | **Spec Completeness** | After `/especdev:specify` | P1 has happy + sad path. Success criteria defined (≥2 measurable). `[NEEDS CLARIFICATION]` markers identified |
| 2 | **Spec Status** | Before `/especdev:plan-technical` | `@ready` tag present. `@draft` specs CANNOT enter the Plan phase |
| 3 | **Clarify Verification** | After `/especdev:clarify` | Auto-QA report with no ❌ FAIL. All `[NEEDS CLARIFICATION]` resolved. Audit completed |
| 4 | **Plan Approval** | After `/especdev:plan-technical` | Constitution check passes (all ✅). Complete technical context. DBML ratified. Complexity tracking documented |
| 5 | **Task Readiness** | After `/especdev:breakdown` | All tasks with IDs (`T###`). File paths on implementation tasks. Parallelism `[P]` markers assigned. 5 phases present |
| 6 | **Code Merge** | After `/especdev:implement` | Tests green, lint clean, type-check passes, coverage ≥85%, no unrelated diff, cross-phase regression passes |
